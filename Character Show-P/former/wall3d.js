(function () {
	...
	function resizeHandler() {
		// 최대 스크롤할 수 있는 높이 = 전체 높이 - 스크롤바(우리가 보는 창 높이)
		// (스크롤바 마지막 머리까지) =
	...
	window.addEventListener("scroll", function () {
		// 스크롤을 한 비율/
		const scrollPer = pageYOffset / maxScrollValue; //맥스 스크롤 값에서 현재 오프셋 (값이 0부터 1까지) 스크롤을 다 내리면 1이 되니깐
		barElem.style.width = scrollPer * 100 + "%";
		const zMove = scrollPer * 980 - 490; //0~1까지 너무 작은값이라 곱하기 980(끝에까지 안가고 살짝 남도록)크게 해준다. (처음 떨어트려놓은 z뷰포트값 490은 뺴줘야한다)
		houseElem.style.transform = "translateZ(" + zMove + "vw)"; //
	});

	//3 마우스 이동하는 곳으로 창 기울기
	window.addEventListener("mousemove", function (e) {
		// clientX,Y 가운데가 0값이 아니다 .시작점이 맨왼쪽 모서리기 떄문에. 그래서 가운데가 0이되도록 식을 만들어 준다. 회전을 쉽게 하기위해
		//     1
		//  -1 0 1 (x축값)
		//    -1
		// 전체 윈도우의 폭분에 현재 마우스의 X 위치 (0~1사이)
	...
	stageElem.addEventListener("click", function (e) {
		...
		new Character({
			...
			xPos: (e.clientX / window.innerWidth) * 100, //클릭한 곳의 x축 위치값
